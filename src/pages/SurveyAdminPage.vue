    ...
    <section v-show="isVisible('overview')" class="section-group">
      ...
      <div class="stats">
        ...
        <div v-for="section in scaleSections" :key="section.key" class="stat-card">
          <strong class="num" :class="tierClass(averages[section.key])">
            {{ averages[section.key] }}
          </strong>
    ...
    <section v-show="isVisible('question-detail')" class="section-group">
      ...
      <div class="panel">
        <div class="header-row header-row--tight">
          ...
          <button
            ...
            @click="questionDetailOpen = !questionDetailOpen"
          >
        ...
        <div v-if="responses.length && questionDetailOpen" class="question-detail">
          <div
            v-for="section in questionStats"
            :key="section.key"
          ...
          >
            ...
            >
              ...
              <div class="question-count-row">
                ...
                >
                  <span class="legend-dot" :class="'score-' + score" />
                  {{ score }} 分：{{ q.dist[score] }}
                </span>

                <span class="question-count question-count--n">
                  n = {{ q.count }}
                </span>
              </div>
            </div>
      ...
      <div class="panel">
        ...
        <div v-if="responses.length" class="stacked-list">
          <div
            v-for="dist in scoreDistributions"
            :key="dist.key"
            class="stacked-row"
          >
    ...
    <section v-show="isVisible('satisfaction')" class="section-group">
      ...
      <div class="panel">
        ...
        <div v-if="responses.length" class="donut-row">
          <svg viewBox="0 0 100 100" class="donut">
            ...
            <text x="50" y="46" text-anchor="middle" class="donut-number">
              {{ issueRate }}%
            </text>

            <text x="50" y="64" text-anchor="middle" class="donut-caption">
              曾遇到問題
            </text>
          </svg>
          ...
          <div class="donut-legend">
            <div class="legend-row">
              <span class="legend-dot issue" />
              曾遇到問題
              <strong class="num">{{ issueRespondentCount }}</strong>
              份
            </div>

            <div class="legend-row">
              <span class="legend-dot no-issue" />
              沒遇到問題
              <strong class="num">{{ responses.length - issueRespondentCount }}</strong>
              份
            </div>
          </div>
        </div>
      ...
      <div class="panel">
        ...
        <p v-else class="empty">
          回覆的日期分布太少，暫時不足以畫出趨勢圖
        </p>
      </div>

      <div v-if="Object.keys(issueTypeDist).length" class="panel">
        <h2>購票問題主要發生在哪些部分</h2>

        <div class="bar-list">
          <div
            v-for="(count, label) in issueTypeDist"
            :key="label"
            class="bar-row"
          >
            ...
            <div class="bar-track">
              <div
                class="bar-fill"
                :style="{ width: pct(count, issueRespondentCount) + '%' }"
              />
            </div>
    ...
    <section v-show="isVisible('demographics')" class="section-group">
      ...
      <div class="panel">
        ...
        <div v-if="Object.keys(activeDemoDist).length" class="bar-list">
          <div
            v-for="(count, label) in activeDemoDist"
            :key="label"
            class="bar-row"
          >
            <span class="bar-label">{{ label }}</span>
            ...
            <div class="bar-track">
              <div
                class="bar-fill"
                :style="{ width: pct(count, responses.length) + '%' }"
              />
            </div>
    ...
    <section v-show="isVisible('text-feedback')" class="section-group">
      ...
      <div class="panel">
        ...
        <ul v-if="improvementAnswers.length" class="text-answers">
          <li v-for="(t, i) in improvementAnswers" :key="i">
            {{ t }}
          </li>
        </ul>
      ...
      <div class="panel">
        ...
        <ul v-if="suggestionAnswers.length" class="text-answers">
          <li v-for="(t, i) in suggestionAnswers" :key="i">
            {{ t }}
          </li>
        </ul>
...
const navSections = [
  ...
  { id: 'satisfaction', label: '購票體驗' },
  { id: 'demographics', label: '使用者資訊' },
...
onMounted(async () => {
  ...
  try {
    ...
    responses.value = snapshot.docs.map((d) => ({
      id: d.id,
      ...d.data()
    }))
  } catch {
...
const issueRespondentCount = computed(
  () =>
    responses.value.filter(
      (r) => r.issueCount && r.issueCount !== '沒有遇到問題'
    ).length
)
...
const issueRate = computed(() => {
  ...
  return Math.round(
    (issueRespondentCount.value / responses.value.length) * 100
  )
})
...
const donutOffset = computed(
  () => donutCircumference * (1 - issueRate.value / 100)
)
...
const trendPoints = computed(() => {
  ...
  return entries.map(([date, count], i) => {
    ...
    const y =
      height -
      padding -
      (count / maxCount) * (height - padding * 2)
...
const improvementAnswers = computed(() =>
  responses.value
    .map((r) => r.improvement)
    .filter(Boolean)
)

const suggestionAnswers = computed(() =>
  responses.value
    .map((r) => r.suggestion)
    .filter(Boolean)
)
